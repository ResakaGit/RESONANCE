...
**Estado: TRACK CERRADO — MG-1 a MG-8 implementados en código (2026-03-25).**

Sprint docs eliminados. Evidencia por sprint:

| Sprint | Descripción | Módulo principal | Estado |
|--------|-------------|-----------------|--------|
| MG-1 | Ecuaciones Termodinámicas | `blueprint/morphogenesis/{thermodynamics,constructal,surface}.rs` | ✅ |
| MG-2 | MetabolicGraph Types | `layers/metabolic_graph.rs` | ✅ |
| MG-3 | Paso Temporal DAG | `simulation/metabolic/morphogenesis.rs` (step + constraint) | ✅ |
| MG-4 | Shape Optimization | `simulation/metabolic/morphogenesis.rs` + `layers/shape_params.rs` | ✅ |
| MG-5 | Albedo Inference | `simulation/metabolic/morphogenesis.rs` + `layers/inferred_albedo.rs` | ✅ |
| MG-6 | Writer Monad + EntropyLedger | `blueprint/equations/metabolic_graph/writer_monad.rs` + `layers/entropy_ledger.rs` | ✅ |
| MG-7 | Surface Rugosity | `simulation/metabolic/morphogenesis.rs` + `layers/morphogenesis_surface.rs` | ✅ |
| MG-8 | Integración + Demo | `entities/archetypes/morphogenesis.rs`, `world/demos/morphogenesis_demo.rs`, `assets/maps/morphogenesis_demo.ron`, `benches/morphogenesis_benchmark.rs` | ✅ |

Plugins: `MetabolicPlugin` (MG-3/6), `MorphologicalPlugin` (MG-4/5/7). Demo: `RESONANCE_MAP=morphogenesis_demo cargo run`.
