...
|--------|---------|--------|------|--------|
| M1 | `SPRINT_M1_ANALYTICS.md` | `src/blueprint/equations/macro_analytics.rs` — `exponential_decay`, `allometric_radius`, `ticks_until_threshold` | 0 | ✅ |
| M2 | `SPRINT_M2_ECS_ROUTING.md` | `MacroStepTarget` component + `apply_macro_step` system in `src/simulation/metabolic/competition_dynamics.rs`, registered in `Phase::MetabolicLayer` | A | ✅ |
| M3 | `SPRINT_M3_NORMALIZATION.md` | Inverse solvers in `src/blueprint/equations/macro_analytics.rs`: `ticks_to_reach`, `normalize_score`, `inverse_normalize_score`, `required_decay_rate` | A | ✅ |
| M4 | `SPRINT_M4_LOD_OBSERVER.md` | `lod_mark_distant_entities` in `src/simulation/metabolic/competition_dynamics.rs`, registered in `Phase::MorphologicalLayer`; `LOD_MACRO_STEP_DIST_SQ`/`LOD_MACRO_STEP_TICKS` constants | B | ✅ |
| M5 | `SPRINT_M5_BENCHMARK.md` | Benchmark suite Euler vs exponential in `src/blueprint/equations/macro_analytics.rs` inline tests | C | ✅ |
| M6 | `SPRINT_M6_BRIDGE_MACRO_CACHE.md` | Audit complete — `exponential_decay` is O(1) and rarely repeated; no BridgeCache needed (documented in `macro_analytics.rs` file header) | D | ✅ |
