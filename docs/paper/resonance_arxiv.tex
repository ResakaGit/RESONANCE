...
We validate internal consistency with 3{,}095 automated tests
across 110{,}000 lines of Rust, demonstrate deterministic
...
(3)~confirmation that combination therapy suppresses more than
double-dose monotherapy (10/10 independent seeds, $p < 0.001$);
and (4)~an adaptive feedback controller that stabilizes tumor
...
\begin{enumerate}
  ...
  \item Seven experimental validations: emergent molecular bonding,
  ...
  \item An open-source, deterministic, 110K-LOC Rust engine
        with 3{,}066 tests and full reproducibility instructions
...
Automated tests & 3{,}095 \\
Source files & 600+ \\
Executable binaries & 25 \\
...
Three new results extend the original framework:
\begin{enumerate}
...
$>10^6$ world-ticks per second with 3{,}095 automated tests.
...
# Full test suite (3066 tests, ~30 sec)
cargo test --release
\end{lstlisting}
