...
fn spawn_canonical_scenario(app: &mut App) -> [Entity; 10] {
    ...
    let child_a3 = app
        ...
        .spawn((
            BaseEnergy::new(0.0),
            PoolParentLink::new(pool_a, ExtractionType::Competitive, 0.7),
        ))
        .id();
    ...
    let child_b1 = app
        ...
        .spawn((
            BaseEnergy::new(0.0),
            PoolParentLink::new(pool_b, ExtractionType::Greedy, 500.0),
        ))
        .id();
    let child_b2 = app
        .world_mut()
        .spawn((
            BaseEnergy::new(0.0),
            PoolParentLink::new(pool_b, ExtractionType::Regulated, 30.0),
        ))
        .id();
    ...
    let child_c1 = app
        ...
        .spawn((
            BaseEnergy::new(0.0),
            PoolParentLink::new(pool_c, ExtractionType::Competitive, 0.4),
        ))
        .id();
    let child_c2 = app
        .world_mut()
        .spawn((
            BaseEnergy::new(0.0),
            PoolParentLink::new(pool_c, ExtractionType::Aggressive, 0.5),
        ))
        .id();

    [
        pool_a, pool_b, pool_c, child_a1, child_a2, child_a3, child_b1, child_b2, child_c1,
        child_c2,
    ]
}
...
fn determinism_pool_distribution_order_stable() {
    fn run_10_children_100_ticks() -> f32 {
        ...
        for i in 0..10 {
            ...
            app.world_mut()
                .spawn((BaseEnergy::new(0.0), PoolParentLink::new(pool, et, param)));
        }
        for _ in 0..100 {
            app.update();
        }
