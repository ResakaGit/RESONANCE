...
use super::protein_fold::{MAX_CHAIN, Monomer};
use crate::blueprint::constants::{
    AMINO_ACID_TYPES, CODON_DELETION_RATE, CODON_DUPLICATION_RATE, CODON_MUTATION_RATE, MAX_CODONS,
    MIN_CODONS, TOTAL_CODONS,
};
...
pub fn crossover_codon(a: &CodonGenome, b: &CodonGenome, rng_state: u64) -> CodonGenome {
    ...
    for i in 0..cut_a.min(new_len) {
        child.codons[i] = a.codons[i];
    }
...
impl CodonTable {
    ...
    pub fn systematic() -> Self {
        ...
        for i in 0..64 {
            mapping[i] = (i as u8 / 8).min(AMINO_ACID_TYPES - 1);
        }
        Self { mapping }
    }
...
pub fn translate_genome(genome: &CodonGenome, table: &CodonTable) -> ([Monomer; MAX_CHAIN], usize) {
    ...
    for i in 0..n_amino {
        let codon_start = i * 3;
        if codon_start + 2 >= genome.len as usize {
            break;
        }
        ...
        let prev_h = if i > 0 {
            chain[i - 1].hydrophobicity
        } else {
            AMINO_HYDROPHOBICITY[amino]
        };
        chain[i] = Monomer {
...
pub fn silent_mutation_fraction(table: &CodonTable) -> f32 {
    ...
    for codon in 0u8..64 {
        ...
        for bit in 0..6u8 {
            ...
            if table.translate(codon) == table.translate(mutant) {
                silent += 1;
            }
            total += 1;
        }
...
mod tests {
    ...
    #[test]
    fn default_has_min_codons() {
        assert_eq!(CodonGenome::default().codon_count(), MIN_CODONS);
    }
    #[test]
    fn default_amino_count() {
        assert_eq!(CodonGenome::default().amino_count(), MIN_CODONS / 3);
    }
    ...
    #[test]
    fn from_seed_codons_in_range() {
        let g = CodonGenome::from_seed(42);
        for &c in g.active() {
            assert!(c < 64, "codon out of range: {c}");
        }
    }
    ...
    #[test]
    fn mutate_preserves_min_len() {
        let g = CodonGenome::default();
        for s in 0..200 {
            assert!(mutate_codon(&g, s).codon_count() >= MIN_CODONS);
        }
    }
    ...
    fn mutate_respects_max_len() {
        ...
        for s in 0..200 {
            assert!(mutate_codon(&g, s).codon_count() <= MAX_CODONS);
        }
    }
    ...
    #[test]
    fn systematic_covers_all_aminos() {
        let t = CodonTable::systematic();
        for amino in 0..AMINO_ACID_TYPES {
            assert!(t.redundancy(amino) > 0);
        }
    }

    #[test]
    fn systematic_uniform_redundancy() {
        let t = CodonTable::systematic();
        for amino in 0..AMINO_ACID_TYPES {
            assert_eq!(t.redundancy(amino), 8);
        }
    }

    #[test]
    fn translate_in_range() {
        let t = CodonTable::systematic();
        for c in 0..64u8 {
            assert!(t.translate(c) < AMINO_ACID_TYPES);
        }
    }

    #[test]
    fn mutate_table_changes_one_entry() {
        let t = CodonTable::systematic();
    ...
    #[test]
    fn translate_hydrophobicity_from_amino() {
        let mut g = CodonGenome::default();
    ...
    #[test]
    fn mutated_table_changes_translation() {
        let g = CodonGenome::from_seed(42);
    ...
    #[test]
    fn classify_same_codon_silent() {
        let t = CodonTable::systematic();
        assert_eq!(classify_mutation(&t, 0, 0), MutationType::Silent);
    }

    #[test]
    fn classify_same_amino_silent() {
        let t = CodonTable::systematic();
    ...
    #[test]
    fn classify_different_amino_missense() {
        let t = CodonTable::systematic();
    ...
    #[test]
    fn silent_fraction_systematic_positive() {
        let f = silent_mutation_fraction(&CodonTable::systematic());
        assert!(
            f > 0.0 && f < 1.0,
            "systematic table should have partial silence: {f}"
        );
    }

    #[test]
    fn silent_fraction_uniform_table_one() {
        let mut t = CodonTable::systematic();
        for i in 0..64 {
            t.mapping[i] = 0;
        } // all map to same amino
        assert!((silent_mutation_fraction(&t) - 1.0).abs() < 1e-5);
    }

    #[test]
    fn neutral_drift_preserves_protein() {
        let g = CodonGenome::from_seed(42);
        let t = CodonTable::systematic();
        ...
        for i in 0..original_len {
            assert!(
                (original_chain[i].hydrophobicity - drifted_chain[i].hydrophobicity).abs() < 1e-5,
