...
use crate::blueprint::constants::{
    ADHESION_COST, ADHESION_FREQ_BANDWIDTH, ADHESION_THRESHOLD, BOND_STRENGTH_SCALE, BORDER_TARGET,
...
pub fn adhesion_affinity(
    ...
    if !freq_a.is_finite() || !freq_b.is_finite() || !distance.is_finite() {
        return 0.0;
    }
    let contact = radius_a + radius_b;
    if distance > contact * 2.0 {
        return 0.0;
    }
...
pub fn detect_colonies(
    ...
    while mask != 0 {
        ...
        mask &= mask - 1;
        let mut mask2 = if i >= 127 {
            0
        } else {
            alive_mask & !((1u128 << (i + 1)) - 1)
        };
        while mask2 != 0 {
    ...
    while mask != 0 {
        ...
        colony_sizes[root_to_label[root] as usize] =
            colony_sizes[root_to_label[root] as usize].saturating_add(1);
    }
...
pub fn modulate_expression(border_sig: f32, current_mask: &[f32; 4], rate: f32) -> [f32; 4] {
    let sig = if border_sig.is_finite() {
        border_sig.clamp(0.0, 1.0)
    } else {
        0.5
    };
...
pub fn specialization_index(masks: &[[f32; 4]], colony_ids: &[u8], target_colony: u8) -> f32 {
...
mod tests {
    ...
    #[test]
    fn affinity_zero_when_far() {
        assert_eq!(adhesion_affinity(400.0, 400.0, 100.0, 1.0, 1.0), 0.0);
    }

    #[test]
    fn affinity_high_when_close_same_freq() {
    ...
    #[test]
    fn affinity_decays_with_freq_diff() {
    ...
    #[test]
    fn bond_strength_positive() {
        assert!(bond_strength(0.8, 50.0, 40.0) > 0.0);
    }

    #[test]
    fn bond_cost_positive() {
        assert!(bond_cost(1.0) > 0.0);
    }

    #[test]
    fn should_bond_threshold() {
    ...
    fn empty_adj() -> [[bool; MAX_ENTITIES]; MAX_ENTITIES] {
        [[false; MAX_ENTITIES]; MAX_ENTITIES]
    }

    #[test]
    fn no_links_no_colonies() {
    ...
    #[test]
    fn border_signal_no_neighbors_one() {
        assert_eq!(border_signal(0, 4), 1.0);
    }
    #[test]
    fn border_signal_all_neighbors_zero() {
        assert_eq!(border_signal(4, 4), 0.0);
    }
    #[test]
    fn border_signal_half() {
        assert!((border_signal(2, 4) - 0.5).abs() < 1e-5);
    }

    #[test]
    fn gradient_non_colony_zero() {
        let adj = empty_adj();
    ...
    #[test]
    fn interior_cell_low_resilience() {
        let mask = [0.5; 4];
        let new = modulate_expression(0.0, &mask, 0.5);
    ...
    #[test]
    fn specialization_index_uniform_zero() {
        let masks = [[0.5; 4]; 3];
        let ids = [1u8, 1, 1];
    ...
    #[test]
    fn specialization_index_diverse_positive() {
        let masks = [
            [1.0, 0.0, 0.5, 0.0],
            [0.0, 1.0, 0.5, 1.0],
            [0.5, 0.5, 0.5, 0.5],
