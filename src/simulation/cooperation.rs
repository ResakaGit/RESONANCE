...
pub fn cooperation_evaluation_system(
    ...
    for &(entity_a, qe_a, rate_a, pos_a, freq_a, phase_a) in snapshot.iter().take(budget) {
        ...
        for entry in nearby.iter().filter(|e| e.entity != entity_a) {
            let Ok(idx_b) = sorted_snapshot
                .binary_search_by_key(&entry.entity.index(), |(e, _, _, _, _, _)| e.index())
            else {
                continue;
            };
            ...
            let interference_cost = apply_metabolic_interference(
                rate_a * COOPERATION_INTERFERENCE_RATE_SCALING,
                1.0 - interference,
            );

            let group_size = 2.0;
            let a_in_group =
                extraction_estimate_in_group(rate_a, group_size, COOPERATION_GROUP_BONUS);
            let b_in_group =
                extraction_estimate_in_group(rate_b, group_size, COOPERATION_GROUP_BONUS);
...
mod tests {
    ...
    fn same_band_entities_propose_alliance() {
        ...
        let idx = build_index(&[
            (e1, bevy::math::Vec2::ZERO),
            (e2, bevy::math::Vec2::new(2.0, 0.0)),
        ]);
        app.insert_resource(idx);
        app.add_systems(Update, cooperation_evaluation_system);
        app.update();

        let events: Vec<AllianceProposedEvent> = app
            .world_mut()
        ...
        assert!(
            !events.is_empty(),
            "same-band entities should propose alliance"
        );
    }
    ...
    fn solo_entity_emits_no_events() {
        ...
        let defects: Vec<_> = app
            ...
            .collect();
        assert!(
            alliances.is_empty() && defects.is_empty(),
            "solo entity: no events"
        );
    }
    ...
    fn cooperation_gain_is_positive_in_event() {
        ...
        if let Some(event) = events.iter().find(|e| e.initiator == e1) {
            assert!(
                event.expected_gain > 0.0,
                "expected_gain should be positive: {}",
                event.expected_gain
            );
        }
    }
