...
pub fn coalition_stability_system(
    ...
    if clock.tick_id % config.eval_interval as u64 != 0 {
        return;
    }
    ...
    for entry in registry.entries.iter_mut() {
        ...
        for (i, &mid) in entry.member_ids[..n].iter().enumerate() {
            if let Some((_, _cm, energy, engine)) = members.iter().find(|(id, _, _, _)| id.0 == mid)
            {
...
pub fn coalition_intake_bonus_system(
    ...
    for (member, mut engine) in &mut members {
        let Some(entry) = registry
            .entries
            .iter()
            .find(|e| e.coalition_id == member.coalition_id)
        else {
            continue;
        };
        ...
        if (engine.intake() - net).abs() > f32::EPSILON {
            engine.set_intake(net);
        }
    }
}
