...
impl CulturalMemory {
    pub fn memes_active(&self) -> &[MemeEntry] { &self.memes[..self.meme_count as usize] }
}
...
impl Default for CultureConfig {
    fn default() -> Self {
        Self { adoption_cost: 1.0, observation_uncertainty: 0.2 }
    }
}
...
pub fn cultural_transmission_system(
    ...
    for (imitator_entity, transform, mut culture, energy, imitator_osc) in &mut imitators {
        ...
        for entry in &nearby {
            ...
            let Some(&(_, t_memes, t_count, t_qe, t_freq, t_phase)) = snapshot.iter()
                .find(|(e, _, _, _, _, _)| *e == target_entity) else { continue };
            ...
            let affinity = culture_eq::frequency_imitation_affinity(
                imitator_freq, imitator_phase, t_freq, t_phase, t,
            );
            ...
            if !culture_eq::should_imitate_with_affinity(
                energy.qe(), t_qe, config.adoption_cost, config.observation_uncertainty,
                affinity, coherence_bonus,
            ) { continue; }

            let t_active = &t_memes[..t_count as usize];
            let best = t_active.iter()
                .filter(|m| !culture.memes_active().iter().any(|own| own.behavior_hash == m.behavior_hash))
                .max_by(|a, b| a.estimated_fitness.partial_cmp(&b.estimated_fitness)
                    .unwrap_or(std::cmp::Ordering::Equal));

            let Some(best_meme) = best else { continue };
            if culture.meme_count as usize >= MAX_MEMES { continue; }
