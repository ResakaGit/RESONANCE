...
use crate::blueprint::constants::{
    CONSTRUCTAL_LIMB_LENGTH_RATIO, CONSTRUCTAL_LIMB_RADIUS_RATIO, CONSTRUCTAL_VISCOSITY,
    MAX_CONSTRUCTAL_LIMBS,
};
use crate::blueprint::equations::optimal_appendage_count;
use crate::geometry_flow::{GeometryInfluence, build_flow_spine};
use crate::layers::{
    AmbientPressure, BodyPlanLayout, CapabilitySet, FlowVector, HasInferredShape, InferenceProfile,
...
pub fn constructal_body_plan_system(
    ...
    for (entity, volume, flow, pressure, shape_opt, profile_opt, caps) in &query {
        ...
        let layout =
            compute_body_plan_layout_from_count(n, &spine, spread, influence.energy_direction);
...
fn compute_body_plan_layout_from_count(
    ...
    let body_center = if spine_len > 0 {
        spine[spine_mid].position
    } else {
        Vec3::ZERO
    };
    let body_tip = if spine_len > 1 {
        spine[spine_len - 1].position
    } else {
        body_center + forward * 0.5
    };
    let body_rear = if spine_len > 0 {
        spine[0].position
    } else {
        body_center - forward * 0.5
    };
    ...
    for i in 0..pairs {
        let t = if pairs > 1 {
            i as f32 / (pairs - 1) as f32
        } else {
            0.5
        };
        let spine_idx =
            ((spine_len as f32 * (0.3 + t * 0.4)) as usize).min(spine_len.saturating_sub(1));
        let anchor = if spine_idx < spine_len {
            spine[spine_idx].position
        } else {
            body_center
        };
...
mod tests {
    ...
    fn four_limbs_produces_bilateral_layout() {
        ...
        assert_eq!(
            layout.active_count(),
            6,
            "expected 6 slots for 4 limbs, got {}",
            layout.active_count()
        );
    }
    ...
    fn odd_limb_count_adds_dorsal() {
        ...
        assert_eq!(
            layout.active_count(),
            5,
            "expected 5 slots for 3 limbs, got {}",
            layout.active_count()
        );
    }
