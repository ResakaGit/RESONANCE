...
use crate::blueprint::equations::pathway_inhibitor::{self as pi, InhibitionMode, Inhibitor};
...
fn apply_pathway_inhibition(
    ...
    while mask != 0 {
        ...
        for j in 0..nc.min(12) {
            if result.effects[j].occupancy <= DISSIPATION_SOLID * 2.0 {
                continue;
            }
        ...
        let diss_increase = result.total_efficiency_loss
            * DISSIPATION_SOLID
            * 200.0
...
fn compute_snapshot(
    ...
    for w in worlds {
        ...
        while mask != 0 {
            ...
            if is_resistant(&w.entities[i], config) {
                rs += 1;
            } else {
                wt += 1;
            }
    ...
    InhibitorSnapshot {
        ...
        selectivity_index: if wt > 0 && rs > 0 {
            rs as f32 / wt as f32
        } else {
            0.0
        },
...
fn spawn_population(world: &mut SimWorldFlat, config: &InhibitorConfig, seed: u64) {
    ...
    for _ in 0..config.resistant_count {
        let (e, vg) = spawn(
            &mut s,
            config.resistant_freq,
            20.0,
            config.wildtype_qe * 0.8,
            0.5,
            8,
        );
        let idx = world.spawn(e);
...
pub fn run(config: &InhibitorConfig) -> InhibitorReport {
    ...
    let mut worlds: Vec<SimWorldFlat> = (0..config.worlds)
        .map(|wi| {
            ...
            for cell in w.irradiance_grid.iter_mut() {
                *cell = config.nutrient_level * 0.3;
            }
    ...
    for generation in 0..config.generations {
        ...
        timeline.push(compute_snapshot(
            &worlds,
            generation,
            config,
            gen_cost,
    ...
    let resistance_gen = timeline.windows(2).find_map(|w| {
        let prev_ratio = if w[0].alive_mean > 0.0 {
            w[0].resistant_alive_mean / w[0].alive_mean
        } else {
            0.0
        };
        let curr_ratio = if w[1].alive_mean > 0.0 {
            w[1].resistant_alive_mean / w[1].alive_mean
        } else {
            0.0
        };
        if curr_ratio > 0.5 && curr_ratio > prev_ratio {
            Some(w[1].generation)
        } else {
            None
        }
    ...
    let pre_drug = timeline
        .iter()
        .find(|s| !s.drug_active)
        .map(|s| s.mean_expression_dim0)
        .unwrap_or(1.0);
    let compensation_detected = timeline
        .iter()
        .rev()
        .take(10)
        .any(|s| s.drug_active && s.mean_expression_dim0 > pre_drug * 0.8);
...
pub fn ablate_concentration(
    base: &InhibitorConfig,
    concentrations: &[f32],
) -> Vec<InhibitorReport> {
    concentrations
        .iter()
...
impl Default for BozicValidationConfig {
    fn default() -> Self {
        Self {
            ...
            drug_a_freq: 400.0,
            drug_a_conc: 0.8,
            drug_a_ki: DISSIPATION_SOLID * 100.0,
            drug_b_freq: 300.0,
            drug_b_conc: 0.8,
            drug_b_ki: DISSIPATION_SOLID * 100.0,
...
fn run_arm(
    ...
    for generation in 0..config.generations {
        ...
        let (mut total_eff, mut count) = (0.0f32, 0u32);
        for w in &worlds {
            let mut mask = w.alive_mask;
            while mask != 0 {
                let i = mask.trailing_zeros() as usize;
    ...
    let nw = worlds.len().max(1) as f32;
    let final_alive: f32 = worlds
        .iter()
        .map(|w| w.alive_mask.count_ones() as f32)
        .sum::<f32>()
        / nw;

    // Resistance = efficiency recovered above 80% of pre-treatment
    let pre = eff_timeline
        .get(drug_active_start.saturating_sub(1) as usize)
        .copied()
        .unwrap_or(1.0);
    let resistance_gen = eff_timeline
        .iter()
        .enumerate()
...
pub fn run_bozic_validation(config: &BozicValidationConfig) -> BozicValidationResult {
    ...
    let drug_a = Inhibitor {
        target_frequency: config.drug_a_freq,
        concentration: config.drug_a_conc,
        ki: config.drug_a_ki,
        mode: InhibitionMode::Competitive,
    };
    let drug_b = Inhibitor {
        target_frequency: config.drug_b_freq,
        concentration: config.drug_b_conc,
        ki: config.drug_b_ki,
        mode: InhibitionMode::Competitive,
    };
    let double_a = Inhibitor {
        target_frequency: config.drug_a_freq,
        concentration: (config.drug_a_conc * 2.0).min(1.0),
        ki: config.drug_a_ki,
        mode: InhibitionMode::Competitive,
    };
...
fn snapshot_tumor(worlds: &[SimWorldFlat], prev_alive: f32) -> pi::TumorSnapshot {
    ...
    for w in worlds {
        ...
        while mask != 0 {
            ...
            count += 1;
            if freq_count < 256 {
                freqs[freq_count] = w.entities[i].frequency_hz;
                freq_count += 1;
            }
        }
    }
...
pub fn run_adaptive(config: &BozicValidationConfig) -> AdaptiveReport {
    ...
    let stability_threshold =
        crate::blueprint::constants::pathway_inhibitor::INHIBITION_DISSIPATION_COST;

    for generation in 0..config.generations {
        // Build inhibitors from current_drugs
        let inhibitors: Vec<Inhibitor> = current_drugs
            .iter()
        ...
        let decision = if drug_active {
            pi::adaptive_decision(
                &snap,
                &current_drugs,
                baseline_eff,
                generation as u64 * config.ticks_per_gen as u64,
            )
        } else {
            pi::TherapyDecision {
                inhibitors: vec![],
                rationale: "pre_treatment",
            }
        };
    ...
    let final_stability = snapshots
        .iter()
        .rev()
        .take(5)
        .all(|s| s.growth_rate.abs() < stability_threshold);
    let stability_gen = snapshots
        .iter()
        .enumerate()
        .find(|(i, _)| {
            if *i < 5 {
    ...
    AdaptiveReport {
        snapshots,
        decisions,
        drug_count_timeline: drug_counts,
        final_stability,
        stability_gen,
        wall_time_ms: start.elapsed().as_millis() as u64,
    }
...
mod tests {
    ...
    fn no_drug_population_survives() {
        ...
        assert!(
            last.alive_mean > 1.0,
            "population should survive without drug: {}",
            last.alive_mean
        );
    }
    ...
    fn drug_suppresses_growth_expression() {
        ...
        if let Some(pre) = pre {
            assert!(
                post.mean_expression_dim0 <= pre.mean_expression_dim0 + 0.1,
                "drug should suppress growth expression: pre={}, post={}",
                pre.mean_expression_dim0,
                post.mean_expression_dim0
            );
        }
    }
    ...
    fn ablation_higher_concentration_more_suppression() {
        ...
        assert!(
            effs[0] >= effs[2] - 0.15,
            "zero drug should have higher efficiency than max: {:?}",
            effs
        );
    }
    ...
    fn inhibition_cost_nonnegative_every_gen() {
        ...
        for snap in &report.timeline {
            assert!(
                snap.total_inhibition_cost >= 0.0,
                "gen {}: cost={} should be >= 0",
                snap.generation,
                snap.total_inhibition_cost
            );
        }
    }
    ...
    fn deterministic_same_seed() {
        ...
        assert_eq!(
            a_last.alive_mean.to_bits(),
            b_last.alive_mean.to_bits(),
    ...
    fn ensemble_different_seeds_different_reports() {
        ...
        let costs: Vec<f32> = reports
            .iter()
            .map(|r| {
                r.timeline
                    .iter()
                    .map(|s| s.total_inhibition_cost)
                    .sum::<f32>()
            })
            .collect();
    ...
    fn bozic_combo_suppresses_more_than_mono() {
        ...
        assert!(
            result.combo_ab.final_efficiency <= result.mono_a.final_efficiency + 0.05,
            "combo should suppress more: combo={}, mono_a={}",
            result.combo_ab.final_efficiency,
            result.mono_a.final_efficiency
        );
    }
    ...
    fn bozic_combo_better_than_double_dose() {
        ...
        let result = run_bozic_validation(&small_bozic());
        assert!(
            result.combo_ab.final_efficiency <= result.double_a.final_efficiency + 0.05,
            "combo should beat double dose: combo={}, double_a={}",
            result.combo_ab.final_efficiency,
            result.double_a.final_efficiency
        );
    }
    ...
    fn bozic_no_drug_highest_efficiency() {
        ...
        assert!(
            ...
            result.no_drug.final_efficiency,
            result.mono_a.final_efficiency
        );
    }
    ...
    fn bozic_all_arms_deterministic() {
        ...
        assert_eq!(
            a.mono_a.final_efficiency.to_bits(),
            b.mono_a.final_efficiency.to_bits()
        );
        assert_eq!(
            a.combo_ab.final_efficiency.to_bits(),
            b.combo_ab.final_efficiency.to_bits()
        );
    }
    ...
    fn hypothesis_config() -> InhibitorConfig {
        InhibitorConfig {
            ...
            resistant_count: 5,
            wildtype_freq: 400.0,
            resistant_freq: 250.0,
            wildtype_qe: 80.0,
            drug_frequency: 400.0,
            drug_concentration: 0.9,
    ...
    fn drug_shifts_population_toward_resistant() {
        ...
        let pre_drug: Vec<&InhibitorSnapshot> = report
            .timeline
            .iter()
            .filter(|s| !s.drug_active && s.alive_mean > 0.0)
            .collect();
        let post_drug: Vec<&InhibitorSnapshot> = report.timeline.iter().rev().take(5).collect();

        if let (Some(pre), Some(post)) = (pre_drug.last(), post_drug.last()) {
            let pre_ratio = if pre.alive_mean > 0.0 {
                pre.resistant_alive_mean / pre.alive_mean
            } else {
                0.0
            };
            let post_ratio = if post.alive_mean > 0.0 {
                post.resistant_alive_mean / post.alive_mean
            } else {
                0.0
            };
    ...
    fn dose_response_controls_efficiency() {
        ...
        assert!(
            effs[0] > effs[2],
            "dose-response: conc=0→eff={}, conc=1→eff={}",
            effs[0],
            effs[2]
        );
    }
    ...
    fn drug_controls_but_does_not_eliminate() {
        ...
        assert!(
            last.alive_mean > 0.0,
            "drug should control, not eliminate: alive={}",
            last.alive_mean
        );
    }
    ...
    fn adaptive_controller_prescribes_therapy() {
        ...
        assert!(
            active_decisions > 0,
            "controller should make active decisions"
        );
        // At least one decision should have a rationale other than pre_treatment
        let has_therapy = report.decisions.iter().any(|d| !d.inhibitors.is_empty());
    ...
    fn adaptive_suppresses_more_than_no_treatment() {
        ...
        assert!(
            adaptive_eff <= no_drug.final_efficiency,
            "adaptive should suppress: adaptive={adaptive_eff}, no_drug={}",
            no_drug.final_efficiency
        );
    }
    ...
    fn adaptive_stabilizes_across_10_seeds() {
        ...
        for seed in 0..10u64 {
            ...
            if last.growth_rate.abs() < 0.1 {
                stable_count += 1;
            }
            // Suppresses = efficiency < 1.0
            if last.mean_efficiency < 0.95 {
                suppresses_count += 1;
            }
        }
        assert!(
    ...
    fn adaptive_vs_fixed_dose() {
        ...
        assert!(
            adaptive_eff <= fixed.final_efficiency + 0.05,
    ...
    fn rosie_config() -> BozicValidationConfig {
        ...
        let (responsive, resistant) =
            fraction_to_entity_counts(45, ROSIE_OBSERVED.responsive_fraction);
    ...
    fn rosie_mono_vaccine_produces_partial_response() {
        ...
        assert!(
            mono.final_efficiency < 0.8,
            "vaccine should reduce efficiency: eff={}",
            mono.final_efficiency
        );
        assert!(
            mono.final_efficiency > 0.05,
            "should NOT eliminate all (resistant fraction): eff={}",
            mono.final_efficiency
        );
    }
    ...
    fn rosie_combo_vaccine_better_than_mono() {
        ...
        let drug_b = Inhibitor {
            target_frequency: cfg.drug_b_freq,
            concentration: cfg.drug_b_conc,
            ki: cfg.drug_b_ki,
            mode: InhibitionMode::Competitive,
        };
        ...
        assert!(
            combo.final_efficiency <= mono.final_efficiency + 0.02,
            "combo should suppress ≥ mono: combo={}, mono={}",
            combo.final_efficiency,
            mono.final_efficiency
        );
    }
    ...
    fn rosie_adaptive_achieves_stability() {
        ...
        let last = report.snapshots.last().unwrap();
        assert!(
            last.growth_rate.abs() < 0.15,
            "adaptive should stabilize: growth_rate={}",
            last.growth_rate
        );
    }
