...
pub fn run(
    ...
    let tops: Vec<GenomeBlob> = reports.iter()
    ...
    let mean = if distances.is_empty() { 0.0 }
               else { distances.iter().sum::<f32>() / distances.len() as f32 };
    let min = distances.iter().copied().fold(f32::MAX, f32::min);
    let max = distances.iter().copied().fold(0.0f32, f32::max);
    let converged = distances.iter().filter(|&&d| d < convergence_threshold).count();
    let convergence_rate = if distances.is_empty() { 0.0 }
                           else { converged as f32 / distances.len() as f32 };
