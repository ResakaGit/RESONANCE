...
pub fn run(preset: &UniversePreset, n_seeds: usize, gens: u32, ticks: u32) -> DebateReport {
    ...
    for i in 0..n_seeds {
        ...
        if final_species > 2.0 {
            life_count += 1;
        }
        if final_diversity > first_div {
            complexity_count += 1;
        }
        if final_diversity > first_div * 1.1 {
            cooperation_count += 1;
        }
