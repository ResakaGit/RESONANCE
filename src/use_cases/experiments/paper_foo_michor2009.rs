...
fn apply_frequency_drift(world: &mut SimWorldFlat, old_mask: u128, seed: u64, mutation_sigma: f32) {
...
fn run_arm(config: &FooMichorConfig, potency: f32, pulsed: bool, arm_seed: u64) -> ArmResult {
    ...
    for generation in 0..config.generations {
        let drug_active = if pulsed {
            ...
            if cycle == 0 {
                ...
                (generation % cycle) < config.pulse_on_gens
            }
        } else {
...
pub fn run(config: &FooMichorConfig) -> FooMichorReport {
    ...
    let dose_resistance_curve: Vec<(f32, f32)> = config
        .dose_levels
        .iter()
        .map(|&dose| {
    ...
    let continuous_08_seed =
        determinism::next_u64(config.seed ^ determinism::hash_f32_slice(&[0.8, 1.0]));
    let continuous_08 = run_arm(config, 0.8 * config.drug_ki, false, continuous_08_seed);

    // Brazo pulsado a 0.8.
    // Pulsed arm at 0.8.
    let pulsed_08_seed =
        determinism::next_u64(config.seed ^ determinism::hash_f32_slice(&[0.8, 2.0]));
    ...
    let (optimal_dose, _min_resistance) =
    ...
    let optimal_exists = if dose_resistance_curve.len() < 3 {
        ...
        let has_increase = dose_resistance_curve
            .windows(2)
            .any(|w| w[1].1 > w[0].1 + 1e-6);
        let has_decrease = dose_resistance_curve
            .windows(2)
            .any(|w| w[1].1 < w[0].1 - 1e-6);
...
mod tests {
    ...
    fn given_same_seed_when_run_twice_then_deterministic() {
        ...
        let a = run(&config);
        let b = run(&config);
        for (i, ((da, ra), (db, rb))) in a
            .dose_resistance_curve
            .iter()
            .zip(b.dose_resistance_curve.iter())
            .enumerate()
        {
    ...
    fn given_resistance_classifier_when_freq_far_then_resistant() {
        assert!(
            is_resistant(500.0, 400.0, 50.0),
            "100 Hz away > 50 bandwidth"
        );
        assert!(
            !is_resistant(420.0, 400.0, 50.0),
            "20 Hz away < 50 bandwidth"
        );
    }
    ...
    fn given_zero_dose_when_run_then_no_resistance() {
        ...
        assert!(
            report.dose_resistance_curve[0].1 <= 1.0,
    ...
    fn given_dose_curve_when_inspected_then_doses_match_config() {
        ...
        for (i, &(dose, _)) in report.dose_resistance_curve.iter().enumerate() {
            assert!(
                (dose - config.dose_levels[i]).abs() < 1e-6,
