...
enum SubPop { Differentiated, Progenitor, Stem }
...
pub fn detect_inflection(timeline: &[f32]) -> Option<(usize, f32, f32)> {
    if timeline.len() < 6 { return None; }

    // Log-transform (clamp > 0).
    let log_vals: Vec<f32> = timeline.iter()
        .map(|&v| (v.max(0.01)).ln())
        .collect();
    ...
    for split in 3..(n - 3) {
        ...
        if slope1 >= 0.0 || slope2 >= 0.0 { continue; }
...
fn spawn_population(world: &mut SimWorldFlat, config: &MichorConfig, seed: u64) {
    let mut s = seed;
    spawn_subpop(world, config.differentiated_count, config.diff_freq, 15.0,
                 config.diff_qe, config.diff_growth_bias, &mut s);
    spawn_subpop(world, config.progenitor_count, config.prog_freq, 10.0,
                 config.prog_qe, config.prog_growth_bias, &mut s);
    spawn_subpop(world, config.stem_count, config.stem_freq, 5.0,
                 config.stem_qe, config.stem_growth_bias, &mut s);
}
...
fn compute_snapshot(
    ...
    let (mut total, mut diff, mut prog, mut stem, mut qe_sum) =
        (0u32, 0u32, 0u32, 0u32, 0.0f32);
...
pub fn run(config: &MichorConfig) -> MichorReport {
    ...
    let treatment_series: Vec<f32> = timeline.iter()
    ...
    let (phase1_slope, phase2_slope, slope_ratio, inflection_gen) = match inflection {
        ...
        None => {
            ...
            let slope = if treatment_series.len() >= 2 {
                linear_slope(&treatment_series.iter().map(|v| v.max(0.01).ln()).collect::<Vec<_>>())
            } else { 0.0 };
    ...
    let stem_survive = timeline.last()
        .map(|s| s.stem_alive > 0.0)
        .unwrap_or(false);
...
mod tests {
    ...
    fn given_drug_drain_when_diff_vs_stem_then_diff_higher() {
        let config = MichorConfig::default();
        let drain_diff = drug_drain(config.diff_freq, config.drug_freq, config.drug_bandwidth, config.drug_potency);
        let drain_stem = drug_drain(config.stem_freq, config.drug_freq, config.drug_bandwidth, config.drug_potency);
        assert!(drain_diff > drain_stem,
    ...
    fn given_linear_slope_when_decreasing_then_negative() {
        ...
        assert!(slope < 0.0, "decreasing values must give negative slope: {slope}");
    }
    ...
    fn given_linear_slope_when_constant_then_zero() {
        ...
        let slope = linear_slope(&vals);
        assert!(slope.abs() < 1e-5, "constant values must give zero slope: {slope}");
    }
    ...
    fn given_biphasic_series_when_inflection_detected_then_ratio_above_1() {
        ...
        assert!(result.is_some(), "should detect inflection in biphasic data");
        let (split, s1, s2) = result.unwrap();
        assert!(split > 3 && split < 17, "split should be near middle: {split}");
        assert!(s1 < 0.0 && s2 < 0.0, "both slopes negative: s1={s1}, s2={s2}");
    }
    ...
    fn given_no_drug_when_run_then_population_stable_or_grows() {
        ...
        let report = run(&config);
        let first = report.timeline.first().map(|s| s.total_alive).unwrap_or(0.0);
    ...
    fn given_snapshot_when_computed_then_subpop_counts_sum() {
        ...
        assert!((snap.total_alive - total_sub).abs() < 0.01,
