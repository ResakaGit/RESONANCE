...
pub fn print_report(report: &ExperimentReport) {
    ...
    println!("╠══════════════════════════════════════════╣");
    println!(
        "║  Seed: {:<10} Worlds: {:<6} Gens: {:<4}║",
        report.seed, report.world_count, report.generations
    );
    println!(
        "║  Ticks/gen: {:<6} Time: {:<6}ms        ║",
        report.ticks_per_gen, report.wall_time_ms
    );
    println!("╚══════════════════════════════════════════╝");
    println!();
    ...
    if let Some(last) = report.history.last() {
        ...
        println!(
            "  Complexity: genes={:.1} metabolic={:.0}% protein={:.0}%",
...
pub fn print_fermi(report: &FermiReport) {
    ...
    println!(
        "║  With life:        {:<4} ({:.1}%)          ║",
        report.with_life,
        report.life_probability * 100.0
    );
    println!(
        "║  With complex life:{:<4} ({:.1}%)          ║",
        report.with_complex_life,
        report.complex_probability * 100.0
    );
    println!("╚══════════════════════════════════════════╝");
}
...
pub fn print_speciation(report: &SpeciationReport) {
    ...
    println!(
        "║  Speciated:  {:<27}║",
        if report.speciated { "YES" } else { "NO" }
    );
    println!("╚══════════════════════════════════════════╝");
}
...
pub fn print_debate(report: &DebateReport) {
    ...
    println!(
        "║  Life emerged:   {:.1}%{:<19}║",
        report.life_rate * 100.0,
        ""
    );
    println!(
        "║  Complexity grew:{:.1}%{:<19}║",
        report.complexity_rate * 100.0,
        ""
    );
    println!(
        "║  Cooperation:   {:.1}%{:<20}║",
        report.cooperation_signal * 100.0,
        ""
    );
    println!("╚══════════════════════════════════════════╝");
}
...
pub fn print_fossil(record: &FossilRecord) {
    ...
    for (i, f) in record.fossils.iter().enumerate() {
        if i % step == 0 || i == record.fossils.len() - 1 {
            println!(
                "  gen {:>4}: {:<5} fit={:.3} div={:.3} spp={:.1}",
                f.generation,
                archetype_label(f.genome.archetype),
                f.fitness,
                f.diversity,
                f.species
            );
...
pub fn print_versus(result: &VersusResult) {
    ...
    println!("╠══════════════════════════════════════════╣");
    println!(
        "║  Team A: {} genomes   qe={:<14.1}║",
        result.survivors_a, result.qe_a
    );
    println!(
        "║  Team B: {} genomes   qe={:<14.1}║",
        result.survivors_b, result.qe_b
    );
