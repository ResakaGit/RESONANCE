...
use crate::bridge::config::{
    BridgeConfig, CatalysisBridge, CollisionTransferBridge, CompetitionNormBridge,
    InterferenceBridge, OsmosisBridge,
};
...
use crate::bridge::macros::impl_bridgeable_scalar_io;
...
/// Input para `normalize_score` — raw_score se band-quantiza; midpoint y k son constantes per-game.
#[derive(Clone, Copy, Debug, PartialEq)]
...
impl Bridgeable for CompetitionNormBridge {
    type Input = CompetitionNormEquationInput;
    type Output = f32;
    ...
    fn cache_key(normalized: Self::Input) -> u64 {
        hash_inputs(&[
            f32::to_bits(normalized.raw_score) as u64,
            f32::to_bits(normalized.midpoint) as u64,
            f32::to_bits(normalized.k) as u64,
        ])
    }

    fn compute(normalized: Self::Input) -> Self::Output {
        equations::normalize_score(normalized.raw_score, normalized.midpoint, normalized.k)
    }
...
mod tests {
    ...
    #[test]
    fn competition_norm_bridge_disabled_bit_exact() {
    ...
    fn competition_norm_bridge_enabled_matches_normalized_equation() {
        ...
        assert!((b - expected).abs() < 1e-5, "b={b} e={expected}");
    }
    ...
    fn competition_norm_bridge_cache_hit_same_band() {
        ...
        assert!(cache.stats().hits >= 1, "expected hit for same band");
    }
