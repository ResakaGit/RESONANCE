...
use crate::bridge::config::{
    ...
    DensityBridge, DissipationBridge, DragBridge, EngineBridge, InterferenceBridge, OsmosisBridge,
...
fn hysteresis_for(preset: RigidityPreset, base: f32) -> f32 {
    match preset {
...
fn cache_cap_for(preset: RigidityPreset, base: usize) -> usize {
    match preset {
...
fn policy_for(preset: RigidityPreset) -> CachePolicy {
    match preset {
...
fn bands_for_equation_preset(moderate: &[BandDef], preset: RigidityPreset) -> Vec<BandDef> {
    match preset {
...
macro_rules! impl_bridge_defaults {
    ($ty:ty, $key:literal, $moderate_bands:expr, $h_base:expr, $cap_base:expr) => {
        impl BridgeDefaults for $ty {
            const FILE_KEY: &'static str = $key;

            fn config_for_preset(preset: RigidityPreset) -> BridgeConfig<Self> {
                let bands = bands_for_equation_preset(&$moderate_bands, preset);
                let h = hysteresis_for(preset, $h_base);
                let cap = cache_cap_for(preset, $cap_base);
                let policy = policy_for(preset);
                let (enabled, rigidity) = match preset {
                    RigidityPreset::Transparent => (false, Rigidity::Transparent),
                    _ => (true, Rigidity::from(preset)),
                };
                finish_config::<Self>(bands, h, cap, policy, enabled, rigidity)
            }
        }
    };
}

// Bandas *Moderate* por ecuación — alineadas a `blueprint_layer_bridge_optimizer.md` §12–13.
