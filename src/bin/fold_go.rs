...
fn main() {
    ...
    println!("  Protein: villin HP35, {} residues", native.n_residues());

    // 2. Build Go topology
    let sequence = native.sequence();
    let bandwidth = 50.0;
    let topo = go_model::build_go_topology(
        &native.ca_positions, &sequence, 8.0, bandwidth, 1.0, 100.0,
    );
    ...
    let q_native = go_model::native_contact_fraction(&native.ca_positions, &topo.native_contacts, 1.2);
    println!("  Q at native: {:.3}", q_native);

    // 4. Extended chain initial structure
    let initial = go_model::extended_chain(native.n_residues(), topo.bond_length);
    let q_extended = go_model::native_contact_fraction(&initial, &topo.native_contacts, 1.2);
    ...
    // Compare Rg
    let rg_native = md_analysis::radius_of_gyration(&native.ca_positions, &vec![1.0; 35]);
    let rg_folded = md_analysis::radius_of_gyration(&result.best_positions, &vec![1.0; 35]);
