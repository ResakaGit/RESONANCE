...
fn main() {
    ...
    let ablation = pathway_inhibitor_exp::ablate_concentration(&config, &[0.0, 0.4, 0.8]);
    for (i, r) in ablation.iter().enumerate() {
        let last = r.timeline.last().unwrap();
        println!(
            "  conc={:.1} → alive={:.1}, efficiency={:.3}, resistance={}",
            [0.0, 0.4, 0.8][i],
            last.alive_mean,
            last.mean_efficiency,
