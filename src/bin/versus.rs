...
fn main() {
    ...
    if let (Some(a), Some(b)) = (&path_a, &path_b) {
        ...
        if let Some((ga, gb)) = resonance::use_cases::experiments::versus::load_competitors(
    ...
    let report_a = resonance::use_cases::evolve_with(
        &resonance::use_cases::presets::EARTH, seed_a as u64, 100, gens as u32, ticks as u32, 12,
    );
    println!("  Evolving Team B (seed={seed_b})...");
    let report_b = resonance::use_cases::evolve_with(
        &resonance::use_cases::presets::EARTH, seed_b as u64, 100, gens as u32, ticks as u32, 12,
    );

    let result = resonance::use_cases::experiments::versus::compare_potential(
        &report_a.top_genomes, &report_b.top_genomes,
    );
    println!();
    resonance::use_cases::presenters::terminal::print_versus(&result);
    println!();
