...
fn main() {
    ...
    if pv1_pass {
        println!("PASS  (TTP ratio: {:.2}×, cycles: {}, drug exposure: {:.0}%)",
            zhang.ttp_ratio, zhang.adaptive_cycles,
            zhang.drug_exposure_ratio * 100.0);
        passed += 1;
    } else {
        println!("FAIL  (adaptive TTP {:?} vs continuous {:?})",
    ...
    if pv2_pass {
        println!("PASS  (persister frac: {:.1}%, survivors: {:.1}, recovery: {})",
    ...
    if pv3_pass {
        println!("PASS  (n=2 within IQR: {}, within 1σ: {})",
    ...
    if pv4_pass {
        ...
        println!("FAIL  (pulsed: {:.1}% vs continuous: {:.1}%)",
            foo.pulsed_resistance_at_08 * 100.0,
            foo.continuous_resistance_at_08 * 100.0);
        failed += 1;
    }
    ...
    if pv5_pass {
        println!("PASS  (slope ratio: {:.1}×, inflection gen: {}, stem survive: {})",
            michor.slope_ratio,
            michor.inflection_gen.map(|g| format!("{g}")).unwrap_or("-".into()),
            michor.stem_survive);
        passed += 1;
    } else {
        println!("FAIL  (biphasic: {}, stem survive: {}, ratio: {:.1})",
    ...
    println!("║  PV-1  Zhang 2022      adaptive TTP > continuous     {}      ║",
        if zhang.prediction_met { "  PASS" } else { "  FAIL" });
    println!("║  PV-2  Sharma 2010     persisters survive + recover  {}      ║",
        if pv2_pass { "  PASS" } else { "  FAIL" });
    println!("║  PV-3  GDSC/CCLE       Hill n=2 within IQR          {}      ║",
        if pv3_pass { "  PASS" } else { "  FAIL" });
    println!("║  PV-4  Foo & Michor    pulsed beats continuous       {}      ║",
