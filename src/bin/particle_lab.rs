...
fn main() {
    ...
    println!(
        ...
        config.positive_count, config.negative_count
    );
    println!(
        "║  Freq spread: {:<6.0} Hz  Arena: {:<4.0}             ║",
        config.freq_spread, config.arena_size
    );
    println!(
        "║  Snapshots: {:<6} Ticks/snap: {:<6}            ║",
        config.snapshots, config.ticks_per_snapshot
    );
    println!("╚══════════════════════════════════════════════════╝\n");
    ...
    println!(
        "  Final bonds: {}",
        report.timeline.last().map(|s| s.bond_count).unwrap_or(0)
    );
    println!(
        "  Final molecule types: {}",
        report
            .timeline
            .last()
            .map(|s| s.molecule_types)
            .unwrap_or(0)
    );
    println!("  Final molecules:");
    for (i, m) in report.final_molecules.iter().enumerate() {
        println!(
            "    #{i}: {} particles, charge={:.1}, freq={:.0} Hz",
